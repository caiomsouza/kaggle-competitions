...
La unica restricción es que la var dep tieen que estar codificada como 0 y 1. Hace una partición en datos train y datos test.

Si se cambia algo hay que hacerlo en el if y el else.
*/

%macro neuralbinariabasica(archivo=,listconti=,listclass=,vardep=,nodos=,corte=,semilla=,porcen=); /*semilla para aplicar a datos train y test*/
...
Crea un 80% de train y 20% de datos test. La tasa de fallos que se obtiene es sobre datos test q es lo que queremos

Dentro de la macro podemos modificar todo lo que queramos, metodo, nodos, etc*/
...
%variar(seminicio=12345,semifin=12350,inicionodos=2,finalnodos=15,increnodos=3);*/


/* MACRO VARIARBIS

UTILIZO LA MISMA MACRO PERO VARIANDO LA SEMILLA Y EL PUNTO DE CORTE, Y 
CONSERVANDO LA INFORMACIÓN PARA UN BOXPLOT  (ESTO NO MERECE MUCHO LA PENA*/
...
/******************************************************************
 MISMAS IDEAS CON PROC LOGISTIC Esto es solo con regresión logística no hay validación cruzada
/******************************************************************/
...
/******************************************************************
REPETIR CON DIFERENTES SEMILLAS Y PONER BOXPLOT COMPARATIVO de la regresión y la red
/******************************************************************/
