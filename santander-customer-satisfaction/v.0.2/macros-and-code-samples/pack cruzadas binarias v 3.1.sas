...
********************************************************************
********************************************************************/
%macro cruzadakNNbin(archivo=,vardepen=,listconti=,ngrupos=,seminicio=,semifinal=,k=,directorio=c:);
data final;run;
...
%macro cruzadaSVMbin(archivo=,vardepen=,listclass=,listconti=,ngrupos=,seminicio=,semifinal=,kernel=linear,c=10,directorio=c:);
data final;run;
*proc printto print="&directorio\ca.txt" log="&directorio\loga.txt";run;
%do semilla=&seminicio %to &semifinal;/*<<<<<******AQUI SE PUEDEN CAMBIAR LAS SEMILLAS */
data dos;set &archivo;u=ranuni(&semilla);
...
age amount checking duration employed installp resident savings 

cualitativas

telephon history other job property 
...
data uno;set discoc.germanbien;run;
proc print data=uno;run;

%cruzadabinarianeural
(archivo=uno,
vardepen=bad,
conti=age amount checking duration employed installp resident savings,
categor=telephon history other job property,
...
listcategor=telephon history other job property,
ngrupos=4,
sinicio=12345,sfinal=12385,
...
listconti=age amount checking duration employed installp resident savings,
listcategor=telephon history other job property,
maxtrees=100,variables=10,porcenbag=0.60,maxbranch=2,tamhoja=5,maxdepth=10,pvalor=0.2,
ngrupos=4,sinicio=12345,sfinal=12385,objetivo=tasafallos);
...
iteraciones=1000,shrink=0.02,maxbranch=2,maxdepth=4,mincatsize=15,minobs=15,objetivo=tasafallos);
...
model bad2=telephon history other job property age amount checking duration employed installp resident savings;
run;
proc contents data=unodummy;run;

%cruzadakNNbin(archivo=unodummy,vardepen=bad2,
listconti=col2-col27,
...
vardepen=bad,
listconti=age amount checking duration employed installp resident savings,
listclass=telephon history other job property,
ngrupos=4,seminicio=12345,semifinal=12385,kernel=polynom k_par=8,c=100);
proc print data=final;run;
