...
%macro cruzadaneural(archivo=,vardepen=,conti=,categor=,ngrupos=,sinicio=,sfinal=,ocultos=3,algo=levmar,acti=tanh,early=,directorio=);
...
/* ESTO ES PARA EARLY STOPPING (maxiter=numero de iteraciones limitado)*/
	...
	%if &early ne %then %do;
		train maxiter=&early /* early stopping cambiar maxiter=25 por ejemplo */ outest=mlpest technique=&algo;%end;
		%else %do;train maxiter=100 /* early stopping cambiar maxiter=25 por ejemplo */ outest=mlpest technique=&algo/* bprop mom=0.2 learn=0.1*/;%end;
...
data _null_;meto=scanq(&lista,&i);call symput('meto',left(meto));run;
 %cruzadaneural(archivo=uno,vardepen=y,conti=x z,categor=clase,ngrupos=3,sinicio=12345,sfinal=12347,ocultos=10,algo=&meto);
...
data _null_;activa=scanq(&lista2,&j);call symput('activa',left(activa));run;
 %cruzadaneural(archivo=uno,vardepen=y,conti=x z,categor=clase,ngrupos=3,sinicio=12345,sfinal=12347,ocultos=10,algo=&meto,acti=&activa);
